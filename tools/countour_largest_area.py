...
gwash = cv2.imread(sys.argv[1]) #import image
...
ret,thresh1 = cv2.threshold(gwashBW,128,255,cv2.THRESH_BINARY) #the value of 15 is chosen by trial-and-error to produce the best outline of the skull
kernel = np.ones((1,1),np.uint8) #square image kernel used for erosion
erosion = cv2.erode(thresh1, kernel,iterations = 80) #refines all edges in the binary image
...
cnt = contours[50] #largest area contour
