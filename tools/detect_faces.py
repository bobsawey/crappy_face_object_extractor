...
def detect_faces(image_path):
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > args_confidence:
            ...
            #cv2.rectangle(ima_rect, rectangle=faces.get_rects(image_path)
            net = cge, (startX, startY), (endX, endY),
                #(0, 0, 255), 1)
            padding = 30
...
def get_rects(image_path):
    ...
    args_prototxt = './tools/models/detect_faces.deploy.prototxt.txt'
    args_model = './tools/models/detect_faces.res10_300x300_ssd_iter_140000.caffemodel'
    args_confidence = 0.9;
    net = cv2.dnn.readNetFromCaffe(args_prototxt, args_model)
    # load the input image and construct an input blob for the image
    # by resizing to a fixed 300x300 pixels and then normalizing it
    ...
    for i in range(0, detections.shape[2]):
        ...
        if confidence > args_confidence:
            ...
            if startX > 500:
                return rect_tuple, tup_tup, False
            ...
            print(f'START X before DIVIDING BY 4={startX}')
    ...
    return rect_tuple, tup_tup, True
